...
| 2026-05-29 | §17.4 "Locus" subsection — wire-shape diagnostic correction. T-attr-2a's original §17.4 amend (row above) pointed at `run_loop::build_audit_payload` + `rpc::enqueue_audit_event` as the `{}`-substitution offenders. Closer investigation showed the daemon-side constructors already produce contract-conformant JSON (`null` for absent optionals, `[]` for empty arrays). The broker-side parse in `dl-satan-attribute-listener--claim-row` was the offender — `:array-type 'list :null-object nil` collapsed both to elisp `nil`, then `json-serialize` re-emitted `nil` as `{}`. Bug fixed in `.emacs.d` commit `c263444` (listener parse → `:array-type 'array :null-object :null`; `dl-satan-audit--iv-require-array` widened to accept vectors; `dl-satan-audit--validate-attribute-caps` switched to `seq-doseq`). Daemon §10.5 from-zero rebuild also landed (separate fix from §17.4 but covered the second daemon-pin) in `~/dev/satan-attrd` commit `fb2b33d`. | Post-T-attr-2a investigation + bug-fix commits. |
...
| 2026-05-29 | T-attr-2d Q7 resolved → **option A** (`satan_attribute_settings` persistent table). §15 Q7 flipped resolved (strikethrough + decision text); §17.5 "Open: decay path" rewritten as normative "Decay path" paragraph specifying: migration `0012_attribute_settings.sql` shape (`name TEXT PK, value JSONB NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()`); broker write-on-toggle via `add-variable-watcher` on `dl-satan-attribute-updates-enabled` plus first-load seeding from the defcustom default; daemon `DecayScheduler::tick` SELECTs the row at tick start and threads the bool into `MaintenanceInput.enabled`, which `dispatch_maintenance` stamps onto `EventInsert.disabled`. The §17.5 apply rule maps unchanged for `disabled=false` (write + UPSERT + audit + bump `last_decay_at`) and `disabled=true` (write + audit, **skip** UPSERT, **skip** bump so the next-enabled tick still fires). Rejected: (B) `pg_notify` + LISTEN cache (over-engineering for a bool that flips ≤ once/day); (C) skip-disable-in-v1 (capsule "disabled" would lie while values drifted downward). T-attr-2d code-bearing PRs ship the migration + helpers + scheduler integration + broker hook in eight trunk commits across `~/dev/satan-attrd` (daemon) and `~/.emacs.d` (broker) per the per-task plan in this session's task list. | T-attr-2d pre-flight resolution (this commit; contract amend only — code lands in subsequent commits). |
...
- **Broker side** (`dl-satan-attribute-listener--claim-row`): the offender. Pre-fix parse used `:array-type 'list :null-object nil`, collapsing both JSON `null` and `[]` to elisp `nil`; `json-serialize` then re-emitted `nil` as `{}` on the transcript-write path. Fixed 2026-05-29 by switching the parse to `:array-type 'array :null-object :null`; vectors round-trip as JSON arrays and `:null` as JSON null. The §5.1 validator now accepts both list- and vector-shaped arrays (`dl-satan-audit--iv-require-array` widened; `dl-satan-audit--validate-attribute-caps` iterates with `seq-doseq`). Reject path (`satan_audit_replies`) was never invoked for this bug — the broker silently rewrote rather than rejected. The reject path is reserved for genuine validator failures.
...
**Restart-while-disabled seq collision (known gap — T-attr-2f).** The per-`maintenance:<utc-day>` `seq` Counter is in-memory and resets on daemon restart (§17.7). On the **enabled** path this is harmless: the first tick bumps `last_decay_at`, so a same-day restart finds nothing due and emits no second event. On the **disabled** path the non-bump above leaves cold targets (`last_decay_at IS NULL`) due across a restart, so the post-restart tick re-emits identical `(run_id, seq)` rows — tripping `UNIQUE (run_id, seq)` (the derived `id` primary key fires first, since `id = <run_id>.attr<NNN>`). The daemon maps either constraint violation to a loud `Error::DecaySeqCollision { run_id, seq }` (`tracing::error!` + abort) rather than corrupting or silently swallowing; **no projection row is mutated**. Recovery is automatic at the next UTC-day boundary (a fresh `run_id` no longer collides); within the same day after such a restart every tick re-collides and logs, until the day rolls or the colliding events are purged. The structural fix — resume the per-day Counter from `MAX(seq)+1` for today's `run_id` on `DecayScheduler` construction — is deferred to **T-attr-2f** (the daily-cadence × disabled × mid-day-restart conjunction is narrow, and the failure is loud and projection-safe, so a test-matrix-plus-guard ships ahead of the structural fix). Verified by `tests/decay.rs:tick_restart_while_disabled_same_day_collision`.
