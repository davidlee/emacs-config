...
7. **Daemon-side decay-disable mechanism (§17.5 + §17.8).** §17.5's "broker stamps `:enabled` per payload" model breaks for decay — daemon-originated events have no incoming payload. Three options surfaced at T-attr-2d pre-flight (see [`../refactor/T-attr-2-decay.md`](../refactor/T-attr-2-decay.md) §"Decay application (T-attr-2d)"): (A) persistent `satan_attribute_settings` table the broker writes on toggle + the daemon scheduler reads per tick; (B) `pg_notify` push channel with in-memory cache on the daemon; (C) skip disable for decay in v1 (operator stops decay by stopping the daemon). Recommendation: (A) — smallest persistent surface, no real-time races, fits §17.5's "daemon-side check" intent literally. Block T-attr-2d on this decision; carry into a §17.5 amend if (A) or (B).
...
**Open: decay path.** §17.8 idle-decay events are daemon-originated — no broker payload to stamp `:enabled` on. The §17.5 model as written assumes every source event is broker-enqueued. The decay-disable mechanism is the subject of open Q §15 Q7 — three options surfaced at T-attr-2d pre-flight; resolution will either amend this section (options A/B) or carve a decay-specific exception (option C).
