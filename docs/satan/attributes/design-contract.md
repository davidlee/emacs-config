...
7. ~~**Daemon-side decay-disable mechanism (§17.5 + §17.8).**~~ Resolved 2026-05-29 → option A: persistent `satan_attribute_settings(name TEXT PRIMARY KEY, value JSONB NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())` table at migration slot `0012_attribute_settings.sql`. Broker writes `('attribute_updates_enabled', true|false)` on every `dl-satan-attribute-updates-enabled` toggle via `add-variable-watcher` (seeded on broker first-load from defcustom default `t`); daemon `DecayScheduler::tick` SELECTs the row at tick start and threads the bool into `MaintenanceInput.enabled`, which `dispatch_maintenance` stamps onto `EventInsert.disabled`. The §17.5 apply rule maps unchanged: `disabled=false` → write event + UPSERT + audit RPC + bump `last_decay_at`; `disabled=true` → write event with `disabled=true`, **skip** UPSERT, audit RPC, **skip** bump (so next-enabled tick still fires). Normative wording lives in §17.5 "Decay path"; that section is the source of truth — this row is the index entry. T-attr-2d ships the migration + helpers + scheduler integration + broker write hook. Rejected: (B) `pg_notify` + LISTEN cache (over-engineering for a single boolean that flips ≤ once/day; still needs startup query); (C) skip-disable-in-v1 (capsule "disabled" render would lie while values drifted downward — observability mismatch).
