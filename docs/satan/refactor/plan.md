...
| [T-attr-2](T-attr-2-decay.md) | Attribute layer — idle decay (daemon-side scheduler) | in-progress | T-attr-1b | 2a contract amend merged 2026-05-29 (bundled §8 decay + §10.5 rebuild idempotence + §17.4 wire-shape); broker JSON wire-shape fix `c263444` + daemon rebuild from-zero `fb2b33d` landed same day; 2b daemon `58e7bba`; 2c daemon `d7f8b89` (Clock + decay.rs + tokio::time::interval, no firing yet); **next: T-attr-2d** firing — blocked on **§15 Q7 (daemon-side decay-disable mechanism)** per [`T-attr-2-decay.md`](T-attr-2-decay.md) §"Decay application (T-attr-2d)". Three options recorded (A persistent settings table / B pg_notify push / C skip-disable v1); decision shapes the PR bundle |
