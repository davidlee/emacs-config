...
| [T-attr-2](T-attr-2-decay.md) | Attribute layer — idle decay (daemon-side scheduler) | in-progress | T-attr-1b | 2a contract amend merged 2026-05-29 (bundled §8 decay + §10.5 rebuild idempotence + §17.4 wire-shape); broker JSON wire-shape fix `c263444` + daemon rebuild from-zero `fb2b33d` landed same day; 2b migration + projection field merged in daemon `58e7bba`; 2c scheduler skeleton (Clock trait + decay.rs + tokio::time::interval, no firing yet) merged in daemon `d7f8b89`; **next: T-attr-2d** firing — extend `DecayScheduler::tick` to dispatch synthetic `(maintenance, idle_decay)` events + bump `last_decay_at`, honour §17.5 disable |
