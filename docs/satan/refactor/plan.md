...
| [T-attr-2](T-attr-2-decay.md) | Attribute layer — idle decay (daemon-side scheduler) | in-progress | T-attr-1b | 2a contract amend merged 2026-05-29 (bundled §8 decay + §10.5 rebuild idempotence + §17.4 wire-shape); broker JSON wire-shape fix `c263444` + daemon rebuild from-zero `fb2b33d` landed same day; **next: T-attr-2b** `0011_attribute_decay.sql` migration + backfill |
