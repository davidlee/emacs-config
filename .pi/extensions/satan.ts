/**
 * DE-007 Phase 3 — pi Extension: SATAN tools over MCP/UDS
 *
 ...
 *
 * Socket: $XDG_RUNTIME_DIR/satan/mcp/mcp.sock (match dl-satan-mcp-runtime-dir
 * + dl-satan-mcp-socket-filename). Override via SATAN_MCP_SOCKET env.
 */
...
const SOCKET_PATH =
  ...
  (process.env.XDG_RUNTIME_DIR
    ? `${process.env.XDG_RUNTIME_DIR}/satan/mcp/mcp.sock`
    : null);

if (!SOCKET_PATH) {
  throw new Error(
    "SATAN MCP: XDG_RUNTIME_DIR not set and SATAN_MCP_SOCKET not provided",
  );
}
...
//
// SATAN tool :args-schema uses: string, integer, boolean, number, object
...
function jsonSchemaToTypeBox(schema: McpInputSchema): TSchema {
  switch (schema.type) {
    ...
    case "boolean":
      return Type.Boolean();
    case "object": {
      if (!schema.properties) return Type.Any();
...
export default async function satanMcpExtension(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    try {
            ...
            async execute(toolCallId, params, signal, _onUpdate, _ctx) {
              ...
              try {
                ...
                return {
                  content: [{
                    type: "text",
                    text: `SATAN MCP: ${err instanceof Error ? err.message : String(err)}`,
                  }],
                  isError: true,
                  details: {},
                };
      ...
      ctx.ui.notify(
        `SATAN MCP: ${registered} tools registered`,
        "info",
      );
    } catch (err) {
      ctx.ui.notify(
