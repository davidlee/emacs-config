...
code_impacts:
  - path: tests/common/mod.rs
    note: shared_pool self-provisions a per-test disposable DB; epoch-named, age-filtered sweep; current_database guard; delete cleanup_* helpers
  - path: tests/store.rs
    note: drop cleanup_scope/cleanup_run tail calls (keep REBUILD_LOCK, unique_scope)
  - path: tests/decay.rs
...
design_decisions:
  ...
  - id: DEC-2
    note: per-test DB (matches shared_pool's per-test contract); dissolves the rebuild-zeroes-table cross-test race
  - id: DEC-3
    note: admin connection derived from DATABASE_URL by swapping db to postgres
  - id: DEC-4
    note: GC via epoch-named, age-filtered sweep-on-init; lock-free; in-use DROP failure as backstop
  - id: DEC-5
    note: keep redundant intra-test isolation (unique_scope/REBUILD_LOCK/DECAY_TEST_LOCK/snapshot); delete only cleanup_* (smallest correct diff)
  - id: DEC-6
    note: reject sqlx::test — it writes _sqlx_test metadata into the DATABASE_URL db (prod)
