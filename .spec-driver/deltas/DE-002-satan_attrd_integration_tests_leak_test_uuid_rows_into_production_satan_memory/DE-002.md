...
name: "Delta - satan-attrd integration tests leak test:<uuid> rows into production satan_memory"
created: "2026-05-30"
updated: "2026-05-30"
...
- **Key Changes** (decided in [DR-002](./DR-002.md), DEC-1 = strategy C):
  - Harness self-provisions a disposable per-binary test DB (`satan_attrd_test_<bin>_<uuid>`)
    from an admin connection derived from `DATABASE_URL` (db→`postgres`); migrate+seed it.
  - Reclaim via sweep-on-init (`DROP` skips live sibling DBs); no cross-binary teardown relied on.
  - Delete the `cleanup_*` apparatus and tail call-sites — disposal makes panic-leaks impossible.
...
- **Acceptance Criteria**:
  ...
  - Sweep reclaims a stray `satan_attrd_test_` DB left by a prior run (VT-sweep-gc).
  - `just check` green; `satan_attributes` shows no new `test:%` scopes after a full run.
- **Planned Artefacts**: VT-no-prod, VT-sweep-gc (DR-002 §5); finalised in IP-002 phases.
