...
name: "Implementation Plan - satan-attrd integration tests leak test:<uuid> rows into production satan_memory"
created: "2026-05-30"
updated: "2026-05-30"
...
entries:
  - artefact: VT-with-db
    ...
    status: planned
    notes: with_db() rewrites both socket (?host=) and tcp DATABASE_URL forms; P01.
  - artefact: VT-no-prod
    kind: VT
    requirement: ISSUE-003
    status: planned
  ...
  - artefact: VT-suite-green
    ...
    requirement: ISSUE-003
    status: planned
    notes: full integration suite passes with cleanup_* removed; P02.
```
...
- **Active Phase Sheet**: `phases/phase-02.md` (P01 complete; P02 active).
