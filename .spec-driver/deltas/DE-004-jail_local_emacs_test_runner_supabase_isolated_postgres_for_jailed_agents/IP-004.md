...
entries:
  ...
  - artefact: VA-DE004-P03
    ...
    phase: IP-004-P03
    status: planned
    notes: Jailed agent transcript for db-start, db-init, and check-batch PASS with DB-backed tests not skipped.
  - artefact: VH-DE004-ISO
    kind: VH
    requirement: DE-004.AC-003
    phase: IP-004-P04
    status: planned
    notes: Human/user attestation that host /run/postgresql is absent or unreachable from the jail after any bootstrap teardown.
```
