...
verification_alignment:
  ...
  - id: VT-perceive-pure
    impact: new
    note: perceive spawns no process, dispatches no tool, enqueues no attribute event, advances no probe/ingest cursor (forbidden-call spies)
  - id: VT-probe-split
    impact: new
...
broker-run(name):
  ...
  prepare = perceive(prepare, mode, dir)   ; UNCONDITIONAL — no LLM, no effects, no consumption-state mutation
                                            ;   persists percept.json; on error → write terminal "perceive-failed" status, return
...
perceive error writes a terminal status artifact rather than leaving a bare dir.
...
| `satan/dl-satan-broker.el` `--write-budget-denied-run` + session-blocked branch | slim bundle, no `:percept` | mirror perceive's `:percept` into `bundle.json` (observer/baseline consumers read `bundle.json → :percept`, not the sidecar) |
...
| `satan/dl-satan-sensor-{curiosity,content,wpm}.el` | read + charge + advance in one call | factor into pure read-snapshot (perceive) + commit (consume) |
...
| VT-perceive-pure | new | perceive spawns no process, dispatches no tool, enqueues no attribute event, advances no probe/ingest cursor (spy `make-process`, `tool-dispatch`, attr-enqueue, cursor writes) |
| VT-probe-split | new | perceive read-snapshot charges nothing; consume commit charges + advances the probe mark; budget-denied → mark unchanged → no signal lost |
