...
entries:
  - artefact: VT-content-tool
    ...
    status: planned
    notes: "ert over temp content store — recent/get/filter/search + error/empty paths (DR-005 §5)"
  - artefact: VT-content-sensor
    kind: VT
    requirement: DE-005.O2
    status: planned
    notes: "ert watermark advance / backlog count, DEC-5 format, disable switch"
  - artefact: VT-content-rule
    kind: VT
    requirement: DE-005.O3
    status: planned
    notes: "ert panopticon.content emits content_domain:* handles; admittable; deduped"
  - artefact: VA-integration
    kind: VA
    requirement: DE-005.O1
    status: planned
    notes: "just check green in live server post-switch; tool dispatches end-to-end"
```
...
- [x] P02 complete
- [x] P03 complete
- [WIP] P04 in-progress — suite green (961/970, 0 unexpected); CHANGELOG done; only VH `home-manager switch` on Sleipnir outstanding
