...
entries:
  ...
  - artefact: VT-process-e2e
    ...
    requirement: DE-008
    status: failed
    notes: "AUD-005 F-001 — dl-satan-observer/process-{positive-bumps-motive-and-projects,error-on-one-iv-does-not-abort} fail with test DB up: fixtures still feed retired :git_head_changed signal (after :git_state head-change, no :git_commits; motive lacks :project_cwd) while asserting :git_commit_observed. DB-gated → skipped in jail. Blocking; re-open P02."
```
