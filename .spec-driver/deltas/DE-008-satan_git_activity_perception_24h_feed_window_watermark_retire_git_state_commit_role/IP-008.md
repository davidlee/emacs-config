...
entries:
  - artefact: VT-git-window
    ...
    requirement: DE-008
    status: planned
    notes: "Phase 1 — git probe admits a commit outside the 10-min focus window but inside 24h; focus/browser unaffected."
  - artefact: VT-feed-paths-multiday
    kind: VT
    requirement: DE-008
    status: planned
    notes: "Phase 1 — git-feed-paths enumerates every calendar day across a multi-day window; DST fall-back day not skipped/duplicated."
  - artefact: VT-sort-limit
    kind: VT
    requirement: DE-008
    status: planned
    notes: "Phase 1 — git-commits-status sorts by :end_ts before (last filt limit); newest is correct under shuffled append order."
  - artefact: VT-malformed-tolerance
    kind: VT
    requirement: DE-008
    status: planned
    notes: "Phase 1 — one malformed line in one day-file does not blank good commits from sibling files."
  - artefact: VT-git-window-field
    kind: VT
    requirement: DE-008
    status: planned
    notes: "Phase 1 — :git_window_start_at present and distinct from :window_start_at."
  - artefact: VT-commit-observed
    kind: VT
    requirement: DE-008
    status: planned
    notes: "Phase 2 — :git_commit_observed fires for an in-window commit in the motive's repo; no fire for unrelated repo / out-of-window / no project_cwd."
  - artefact: VT-p2-retired
    kind: VT
    requirement: DE-008
    status: planned
    notes: "Phase 2 — :git_head_changed removed from registry + tests; rg ':git_head_changed|git-head-changed' empty outside CHANGELOG history."
  - artefact: VA-live-tick
    kind: VA
    requirement: DE-008
    status: planned
    notes: "Phase 2 — throwaway commit in a tracked repo + forced tick → appears in next percept.json git_commits."
```
