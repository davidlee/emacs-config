---
id: mem.fact.satan.ingest-cursor-store
slug: ingest-cursor-store
name: SATAN per-source ingest cursor store
kind: memory
status: active
memory_type: fact
created: '2026-06-10'
...
provenance:
  sources:
    - kind: code
      ref: satan/dl-satan-ingest-cursor.el
...
- cursor
- perception
summary: 'dl-satan-ingest-cursor.el: per-source evidence frontier (focus/browser end_ts,
  content captured_at; git EXCLUDED; intra-day). Advance=max(current,head), consume-success-only,
  verbatim native ts. Comparator asymmetry intentional: content string<, focus/browser
  parsed-instant.'
---
