...
- jsonl
- gotcha
summary: RESOLVED — root cause fixed by changing `defun` to `cl-defun` in dl-satan-jsonl.el. Stale .elc must be deleted after the change.
  Legacy workaround (use lenient reader or pass 3 args) no longer required.
---
