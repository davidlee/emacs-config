---
id: POL-001
title: 'POL-001: SATAN module extraction policy'
status: required
created: '2026-05-30'
updated: '2026-05-30'
reviewed: '2026-05-30'
...
related_policies: []
related_standards: []
tags: [satan, refactor, extraction]
summary: 'When a SATAN module earns its seat inside the Emacs broker vs. when it should be extracted to a daemon/CLI; the standing test, triggers, and principles.'
---
...
the extraction candidates it evaluates live in the backlog (IMPR-006..009),
...
- **Active beachhead — `satan-attrd`** (attribute layer, first Rust daemon).
  In flight; capsule render = IMPR-003.
- **IMPR-006** patch runner → `satan-patcher` (pivot-pending, in flight).
- **IMPR-007** memory substrate → `satan-memoryd` (biggest editor-mismatch).
- **IMPR-008** audit verifier → `satan-audit` CLI (mechanical, CI-useful).
- **IMPR-009** observer → daemon, folds into IMPR-007 or stands alone.
...
- Backlog: IMPR-003 (attrd capsule), IMPR-006..009 (candidates).
