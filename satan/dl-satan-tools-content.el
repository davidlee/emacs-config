;;; dl-satan-tools-content.el --- content_read tool handler -*- lexical-binding: t; -*-
...
(defcustom dl-satan-tools-content-dir
...
(defcustom dl-satan-tools-content-default-limit 20
  "Default `:limit' for `recent' and `filter' scopes."
  :type 'integer :group 'dl-satan)

(defcustom dl-satan-tools-content-search-limit 10
  "Default `:limit' for `search' scope."
  :type 'integer :group 'dl-satan)

(defcustom dl-satan-tools-content-page-max 5000
  "Maximum characters per `get' page."
  :type 'integer :group 'dl-satan)

(defcustom dl-satan-tools-content-recent-scan-max 500
  "Maximum rows to parse from the tail of `articles.jsonl'.
Bounds I/O for the unbounded index (see DE-005 R6 / DR-005 DEC-6)."
  :type 'integer :group 'dl-satan)

(defcustom dl-satan-tools-content-rg-path nil
  "Path to `rg' binary for search scope.  nil means `executable-find'."
  :type '(choice (const :tag "Auto-detect" nil)
                 (file :tag "Custom path"))
  :group 'dl-satan)

(defconst dl-satan-tools-content--limit-hard-max 200
  "Hard cap on :limit for metadata scopes (recent/filter).")

(defconst dl-satan-tools-content--search-limit-hard-max 50
  "Hard cap on :limit for search scope.")

(defconst dl-satan-tools-content--search-snippet-max 200
  "Maximum characters for a search match snippet.")

;; --- Internal helpers -----------------------------------------

(defun dl-satan-tools-content--articles-path ()
  "Return the full path to `articles.jsonl'."
  (expand-file-name "articles.jsonl" dl-satan-tools-content-dir))

(defun dl-satan-tools-content--sidecar-json-path (hash)
  "Return the path to `<shard>/<hash>.json' for HASH."
  (expand-file-name
   (format "%s/%s.json" (substring hash 0 2) hash)
   dl-satan-tools-content-dir))

(defun dl-satan-tools-content--clamp-limit (raw default hard-max)
...
(defun dl-satan-tools-content--read-json (path)
...
symmetry but currently always uses lenient mode."
  (let ((path (dl-satan-tools-content--articles-path)))
    (when (file-readable-p path)
      (dl-satan-tools-content--read-jsonl-lenient path))))

(defun dl-satan-tools-content--read-jsonl-lenient (path)
...
(defun dl-satan-tools-content--scope-recent (args)
  ...
  (let* ((limit (dl-satan-tools-content--clamp-limit
         ...
         (tail (last scanned (min limit (length scanned)
                                  dl-satan-tools-content-recent-scan-max)))
...
(defun dl-satan-tools-content--scope-get (args)
...
Returns paginated text_content per DR-005 DEC-4."
  (let* ((hash (plist-get args :hash))
         ...
         (limit (dl-satan-tools-content--clamp-limit
                 raw-limit dl-satan-tools-content-page-max
                 dl-satan-tools-content-page-max))
         (articles (dl-satan-tools-content--read-articles-jsonl)))
    ;; Resolve hash -> article metadata
    (cond
     ((null articles)
      (cons 'error "content index unreadable"))
     ((null (dl-satan-tools-content--lookup-article hash articles))
      (cons 'error (format "unknown content_hash: %s" hash)))
     (t
      (let* ((article (dl-satan-tools-content--lookup-article hash articles))
             (sidecar-path (dl-satan-tools-content--sidecar-json-path hash))
             (sidecar (dl-satan-tools-content--read-json sidecar-path)))
        (if (null sidecar)
    ...
    (dolist (a articles)
      (when (and (or (null domain)
        ...
        (let* ((hash (plist-get a :content_hash))
               (sidecar (dl-satan-tools-content--read-json
                         (dl-satan-tools-content--sidecar-json-path hash)))
               (excerpt (when sidecar
                          (dl-satan-tools-content--excerpt sidecar))))
...
(defun dl-satan-tools-content--resolve-rg ()
  "Return the `rg' binary path, or nil."
  (or dl-satan-tools-content-rg-path
      (executable-find "rg")))

(defun dl-satan-tools-content--scope-search (args)
...
Uses `rg' subprocess via `call-process' (never shell-command)."
  (let* ((query (plist-get args :query))
         (limit (dl-satan-tools-content--clamp-limit
                 (plist-get args :limit)
                 dl-satan-tools-content-search-limit
                 dl-satan-tools-content--search-limit-hard-max))
         (rg (dl-satan-tools-content--resolve-rg))
         (root dl-satan-tools-content-dir)
         (articles (dl-satan-tools-content--read-articles-jsonl :skip-malformed t))
         (index (or articles '())))
    (if (null rg)
                     ...
                     (let* ((a (gethash h article-map))
                            ...
                            (snippet
                             (if (> (length raw-snippet)
                                    dl-satan-tools-content--search-snippet-max)
...
(defun dl-satan-tool/content-read (args _ctx)
  "Implements `content_read'.  ARGS: (:scope recent|get|filter|search …).
Returns (ok PLIST) | (error STRING)."
  (let ((scope (plist-get args :scope)))
    (pcase scope
      ("recent"  (dl-satan-tools-content--scope-recent args))
      ("get"     (dl-satan-tools-content--scope-get args))
      ("filter"  (dl-satan-tools-content--scope-filter args))
      ("search"  (dl-satan-tools-content--scope-search args))
...
(dl-satan-tool-register
 (list :name "content_read"
       ...
       :handler 'dl-satan-tool/content-read))

(provide 'dl-satan-tools-content)
;;; dl-satan-tools-content.el ends here
