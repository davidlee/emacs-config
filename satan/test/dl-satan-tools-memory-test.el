;;; dl-satan-tools-memory-test.el --- memory_* tool ert -*- lexical-binding: t; -*-

;; Step 8 of memory.design.md.  Cover the three `memory_*` tools
;; (mark, resonate, show_trace):
;;   - schema validation (delegated to dl-satan-tools.el)
...
(defconst dl-satan-tools-memory-test--db "satan_memory_test")
...
(defconst dl-satan-tools-memory-test--evidence-stub
...
(defmacro dl-satan-tools-memory-test--stub-evidence (&rest body)
  "Stub `dl-satan-memory-evidence-assemble' to return the fixture."
  (declare (indent 0))
...
(defmacro dl-satan-tools-memory-test--with-db (&rest body)
  (declare (indent 0))
  `(progn
     (skip-unless (dl-satan-tools-memory-test--reachable-p))
     (dl-satan-tools-memory-test--reset-and-migrate)
...
(ert-deftest dl-satan-tools-memory/mind-docs-exist ()
...
(ert-deftest dl-satan-tools-memory/registered ()
  (dolist (name '("memory_mark" "memory_resonate" "memory_show_trace"))
...
(ert-deftest dl-satan-tools-memory/ctx-from-tool-ctx ()
...
(ert-deftest dl-satan-tools-memory/ctx-from-tool-ctx-string-mode ()
  (cl-letf (((symbol-function 'dl-satan-tools-memory--now)
             (lambda () "2026-05-19T10:00:00+10:00")))
    (let ((ctx (dl-satan-tools-memory--ctx-from
...
(defun dl-satan-tools-memory-test--dispatch (name args)
...
(ert-deftest dl-satan-tools-memory/mark-missing-payload-rejected ()
  (let ((res (dl-satan-tools-memory-test--dispatch
              "memory_mark" '())))
    (should (eq (plist-get res :ok) :false))
    (should (string-match-p "payload" (plist-get res :error)))))

(ert-deftest dl-satan-tools-memory/mark-bad-hint-kind-rejected ()
...
(ert-deftest dl-satan-tools-memory/mark-bad-valence-rejected ()
  (dl-satan-tools-memory-test--stub-evidence
    (let ((res (dl-satan-tools-memory-test--dispatch
                "memory_mark"
...
(ert-deftest dl-satan-tools-memory/show-missing-trace-id-rejected ()
  (let ((res (dl-satan-tools-memory-test--dispatch
              "memory_show_trace" '())))
    (should (eq (plist-get res :ok) :false))
    (should (string-match-p "trace_id" (plist-get res :error)))))

(ert-deftest dl-satan-tools-memory/resonate-bad-min-score-rejected ()
  (let ((res (dl-satan-tools-memory-test--dispatch
...
(ert-deftest dl-satan-tools-memory/mark-non-string-topic-rejected ()
  (dl-satan-tools-memory-test--stub-evidence
    (let ((res (dl-satan-tools-memory-test--dispatch
                "memory_mark"
...
(ert-deftest dl-satan-tools-memory/mark-bad-link-rejected ()
  (dl-satan-tools-memory-test--stub-evidence
    (let ((res (dl-satan-tools-memory-test--dispatch
                "memory_mark"
                '(:payload "x"
...
(ert-deftest dl-satan-tools-memory/resonate-non-string-kinds-rejected ()
  (let ((res (dl-satan-tools-memory-test--dispatch
              "memory_resonate" '(:kinds (1 2)))))
    (should (eq (plist-get res :ok) :false))
    (should (string-match-p "kinds" (plist-get res :error)))))

(ert-deftest dl-satan-tools-memory/resonate-non-string-cue-handles-rejected ()
  (let ((res (dl-satan-tools-memory-test--dispatch
...
(ert-deftest dl-satan-tools-memory/mark-packages-handles-with-sources ()
  (dl-satan-tools-memory-test--stub-evidence
    (dl-satan-tools-memory-test--capture
        captured dl-satan-memory-store-mark
      (let ((res (dl-satan-tools-memory-test--dispatch
                  "memory_mark"
        ...
        (should (equal dl-satan-memory-grammar-current-version
...
(ert-deftest dl-satan-tools-memory/mark-top-level-valence-wins ()
  (dl-satan-tools-memory-test--stub-evidence
    (dl-satan-tools-memory-test--capture
        captured dl-satan-memory-store-mark
      (dl-satan-tools-memory-test--dispatch
       "memory_mark"
       '(:payload "p" :valence "negative"
         :hints (:valence "positive")))
      (should (equal "negative" (plist-get captured :valence))))))

(ert-deftest dl-satan-tools-memory/mark-hint-valence-fallback ()
  (dl-satan-tools-memory-test--stub-evidence
    (dl-satan-tools-memory-test--capture
        captured dl-satan-memory-store-mark
      (dl-satan-tools-memory-test--dispatch
...
(ert-deftest dl-satan-tools-memory/mark-rejected-pass-through ()
...
(ert-deftest dl-satan-tools-memory/mark-propagates-store-error ()
  (dl-satan-tools-memory-test--stub-evidence
    (cl-letf (((symbol-function 'dl-satan-memory-store-mark)
...
(ert-deftest dl-satan-tools-memory/resonate-explicit-handles ()
  ;; Explicit cue.handles bypasses the evidence assembler.
  (let (evidence-called)
    (cl-letf (((symbol-function 'dl-satan-memory-evidence-assemble)
               (lambda (&rest _) (setq evidence-called t) nil))
              ((symbol-function 'dl-satan-memory-store-resonate)
               (lambda (&rest args)
                 (cons 'ok
                       (list
...
(ert-deftest dl-satan-tools-memory/resonate-derives-from-hints ()
...
(ert-deftest dl-satan-tools-memory/resonate-empty-cue-is-empty ()
...
(ert-deftest dl-satan-tools-memory/resonate-forwards-limit-kinds-min-score ()
  (let (passed)
    (cl-letf (((symbol-function 'dl-satan-memory-store-resonate)
               (lambda (&rest args)
                 (setq passed args)
...
(ert-deftest dl-satan-tools-memory/resonate-propagates-store-error ()
  (cl-letf (((symbol-function 'dl-satan-memory-store-resonate)
...
(ert-deftest dl-satan-tools-memory/show-pass-through ()
  (cl-letf (((symbol-function 'dl-satan-memory-store-show)
             (lambda (tid &rest _)
...
(ert-deftest dl-satan-tools-memory/show-missing-trace-ok-nil ()
  (cl-letf (((symbol-function 'dl-satan-memory-store-show)
             (lambda (&rest _) (cons 'ok nil))))
    (let ((res (dl-satan-tools-memory-test--dispatch
...
(ert-deftest dl-satan-tools-memory/db-mark-show-round-trip ()
  (dl-satan-tools-memory-test--with-db
    (dl-satan-tools-memory-test--stub-evidence
      (let* ((mark-res (dl-satan-tools-memory-test--dispatch
                        "memory_mark"
        ...
        (should (stringp tid))
        ;; show
        (let* ((show-res (dl-satan-tools-memory-test--dispatch
                          "memory_show_trace"
                          (list :trace_id tid)))
...
(ert-deftest dl-satan-tools-memory/db-resonate-finds-mark ()
  (dl-satan-tools-memory-test--with-db
    (dl-satan-tools-memory-test--stub-evidence
      (dl-satan-tools-memory-test--dispatch
...
(provide 'dl-satan-tools-memory-test)
;;; dl-satan-tools-memory-test.el ends here
