;;; dl-satan-resonance.el --- SATAN auto-resonance (Phase 2) -*- lexical-binding: t; -*-
...
;;
;; Public surface:
;;   (dl-satan-resonance-derive PERCEPT &optional OPTS)   -> PLIST or nil
;;   (dl-satan-resonance-render-block FRAMING RESONANCE)  -> LIST-OF-LINES
;;
;; PERCEPT is the plist returned by `dl-satan-percept-build' — supplies
...
(defconst dl-satan-resonance--excluded-rule-ids
...
(defconst dl-satan-resonance--default-limit 3
  "Max matches injected into the capsule per tick (design §S2: top 1–3).")

(defun dl-satan-resonance--admittable-p (sources)
...
That handle is what makes the cue worth resonating on."
  (cl-some (lambda (row)
             (not (member (plist-get row :rule_id)
                          dl-satan-resonance--excluded-rule-ids)))
           sources))

(defun dl-satan-resonance-derive (percept &optional opts)
  "Return the resonance result plist for PERCEPT, or nil to omit the block.
PERCEPT is the build result from `dl-satan-percept-build'.  OPTS forwards
test knobs:
...
psql blip should not fail the run (handover watch-out)."
  (let* ((handles (plist-get percept :handles))
         ...
         (limit (or (plist-get opts :limit)
                    dl-satan-resonance--default-limit))
    ...
    (cond
     ...
     ((not (dl-satan-resonance--admittable-p sources))
...
(defconst dl-satan-resonance--framing-key "resonance_block_header"
...
(defun dl-satan-resonance-render-block (framing resonance)
      ...
      (let ((lines (list header)))
        (dolist (m matches)
          (push (format "- %s  score %s"
                        ...
                        (dl-satan-resonance--score-format
                         (plist-get m :score)))
                lines)
          (push (concat "    matched: "
                        (mapconcat #'identity
...
;;; dl-satan-resonance.el ends here
