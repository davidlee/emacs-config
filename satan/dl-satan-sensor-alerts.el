...
;; `dl-satan-tick-quiet-p' is the default quiet-hours predicate but
;; `dl-satan-tick' itself requires `dl-satan-context' which transitively
;; loads this file; declare the function and resolve at call-site so
;; the require graph stays acyclic.
(declare-function dl-satan-tick-quiet-p "dl-satan-tick" (&optional time))
...
(defcustom dl-satan-sensor-alerts-cooldown-seconds 86400
...
(defcustom dl-satan-sensor-alerts-bough-streak-threshold 3
...
(defconst dl-satan-sensor-alerts--causes
  '((:current_window
...
(defun dl-satan-sensor-alerts--match-kind (status)
...
(defun dl-satan-sensor-alerts--lookup-cause (sensor-key kind)
  "Return the (CAUSE SEVERITY MESSAGE REMEDIATION) tuple for SENSOR-KEY+KIND."
  (let ((rules (cdr (assq sensor-key dl-satan-sensor-alerts--causes))))
    (cdr (assoc kind rules))))

(defun dl-satan-sensor-alerts--derive-causes (sensor-status)
...
:sensor :status).  Returns nil when every sensor is `ok'."
  (cl-loop
   for key in dl-satan-sensor--source-order
   for status = (plist-get sensor-status key)
   for kind = (dl-satan-sensor-alerts--match-kind status)
   for tuple = (and kind (dl-satan-sensor-alerts--lookup-cause key kind))
   when tuple
...
;; ---------------------------------------------------------------------
;; State file I/O (atomic tmp + rename, like dl-satan-audit--write-json)
;; ---------------------------------------------------------------------

(defun dl-satan-sensor-alerts--read-state (path)
...
(defun dl-satan-sensor-alerts--write-state (path state)
...
(defun dl-satan-sensor-alerts--iso->time (s)
...
(defun dl-satan-sensor-alerts--notify-call (cause message)
...
(defun dl-satan-sensor-alerts--dispatch (cause message tool-ctx)
  "Dispatch the alert through `dl-satan-tool-dispatch'.
...
requires notify')."
  (let* ((call (dl-satan-sensor-alerts--notify-call cause message))
...
(defun dl-satan-sensor-alerts--capability-denied-p (reason)
...
(defun dl-satan-sensor-alerts--make-tool-ctx (mode time-now run-dir)
...
(defun dl-satan-sensor-alerts--entry (cause base &rest extras)
...
(cl-defun dl-satan-sensor-alerts-check
...
sensor was `ok')."
  (let* ((now (and time-now (date-to-time time-now)))
         ...
         (path (or state-file dl-satan-sensor-state-file))
         (quiet-p (funcall (or quiet-p-fn #'dl-satan-tick-quiet-p) now))
         (state (dl-satan-sensor-alerts--read-state path))
         (state (dl-satan-sensor-alerts--bump-bough-streak
                 state (plist-get sensor-status :bough)))
         (tool-ctx (dl-satan-sensor-alerts--make-tool-ctx
                    mode now-iso run-dir))
         (causes (dl-satan-sensor-alerts--derive-causes sensor-status))
         entries)
    (dolist (base causes)
      (let* ((cause (plist-get base :cause))
        ...
        (cond
         ...
         ((not (dl-satan-sensor-alerts--cooldown-elapsed-p cs now))
          (setq entry (dl-satan-sensor-alerts--entry
                       cause base :suppressed t :reason "cooldown")))
         (t
          (let* ((res (dl-satan-sensor-alerts--dispatch cause msg tool-ctx))
