;;; dl-satan-motive.el --- SATAN motive file parser (Phase 3) -*- lexical-binding: t; -*-
...
;;
;; Public surface:
;;   (dl-satan-motive-parse  TEXT)         -> PLIST
;;   (dl-satan-motive-read   PATH)         -> PLIST  (silent on missing)
;;   (dl-satan-motive-render-block FRAMING PARSED) -> LIST-OF-LINES
;;   (dl-satan-motive-validate-for-write TEXT)     -> PLIST or nil
;;
...
(defconst dl-satan-motive-max-active 3
  "Hard cap on active motives (§S3 / A7).")

(defconst dl-satan-motive-max-ruminations 10
  "Hard cap on rumination lines (§S3 / A7).")

(defconst dl-satan-motive--handle-regexp
  "\\`[a-z][a-z0-9_]*:[A-Za-z0-9][A-Za-z0-9_.+>-]*\\'"
  "Canon handle regex (mirrors `dl-satan-memory-canon--validate-emission').
A motive `:cue:' entry that does not match is rejected.")

(defconst dl-satan-motive--admitted-namespaces
...
(defconst dl-satan-motive--ruminations-heading "ruminations"
  "Top-level org heading that holds rumination lines (case-folded).")

(defconst dl-satan-motive--ceiling-field "ceiling"
...
(defun dl-satan-motive--split-sections (text)
...
(defun dl-satan-motive--parse-footer-line (line)
...
(defun dl-satan-motive--parse-cue (raw)
...
(defun dl-satan-motive--cue-handles-well-formed-p (handles)
  "Return non-nil when every handle in HANDLES matches the canon regex."
  (and handles
       (cl-every (lambda (h)
                   (and (stringp h)
                        (string-match-p dl-satan-motive--handle-regexp h)))
                 handles)))

(defun dl-satan-motive--cue-admittable-p (handles)
  "Return non-nil when ≥1 handle's namespace is admitted (§S3).
Parallel of `dl-satan-resonance--admittable-p' but inspects the
  ...
  (cl-some (lambda (h)
             (when (and (stringp h) (string-match "\\`\\([a-z][a-z0-9_]*\\):" h))
               (member (match-string 1 h)
                       dl-satan-motive--admitted-namespaces)))
           handles))

(defun dl-satan-motive--motive-id (heading)
...
(defun dl-satan-motive--parse-integer (raw)
...
HEADING is the org heading (without leading `* '); LINES is the body."
  (let ((id (dl-satan-motive--motive-id heading))
    ...
    (dolist (raw-line lines)
      (let ((field (dl-satan-motive--parse-footer-line raw-line)))
        (cond
         (field
          (setq in-footer t)
          (pcase (car field)
            ("cue"
             (setq cue-raw (cdr field)
                   cue (dl-satan-motive--parse-cue (cdr field))))
            ("cooldown_s"
             (setq cooldown (dl-satan-motive--parse-integer (cdr field))))
            ("worked_count"
             (setq worked (or (dl-satan-motive--parse-integer (cdr field)) 0)))
    ...
    (let* ((prose (string-trim
           ...
           (well-formed (dl-satan-motive--cue-handles-well-formed-p cue))
           (admittable (and well-formed
                            (dl-satan-motive--cue-admittable-p cue)))
           (dormant-reason
            (cond
...
(defun dl-satan-motive--parse-ruminations (lines)
...
(defun dl-satan-motive-parse (text)
...
Behaviour:
  ...
  (let ((sections (dl-satan-motive--split-sections (or text "")))
        motives ruminations errors)
    (dolist (cell sections)
      (let* ((heading (car cell))
             (lines (cdr cell)))
        (if (equal (downcase (string-trim heading))
                   dl-satan-motive--ruminations-heading)
            (setq ruminations (dl-satan-motive--parse-ruminations lines))
          (let ((m (dl-satan-motive--parse-motive heading lines)))
...
(defun dl-satan-motive-read (path)
...
(defconst dl-satan-motive--framing-key "motive_block_header"
...
(defun dl-satan-motive--active-motives (parsed)
     ...
     (let ((forbidden (cl-find :forbidden-field parse-errors
       ...
       (when forbidden
         (list :bound :forbidden-field
               ...
               :field dl-satan-motive--ceiling-field)))
     (when (> (length actives) dl-satan-motive-max-active)
       (list :bound :too-many-active
             :limit dl-satan-motive-max-active
             :got (length actives)))
     (when (> (length ruminations) dl-satan-motive-max-ruminations)
...
(defun dl-satan-motive-format-write-error (err)
