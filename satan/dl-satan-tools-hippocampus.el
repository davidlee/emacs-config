;;; dl-satan-tools-hippocampus.el --- hippocampus tools -*- lexical-binding: t; -*-
...
(defun dl-satan-tools-hippocampus--parse-title (filename)
...
(defun dl-satan-tools-hippocampus--entry (path)
  "Build an entry plist for PATH (absolute)."
  (let* ((filename (file-name-nondirectory path))
         (mtime (file-attribute-modification-time (file-attributes path))))
    (list :filename filename
          :title (dl-satan-tools-hippocampus--parse-title filename)
          :mtime (format-time-string "%Y-%m-%dT%H:%M:%S%z" mtime))))

(defun dl-satan-tool/hippocampus-list (_args _ctx)
  "List all hippocampus entries.  Returns (ok :entries [...])."
  (if (not (file-directory-p dl-satan-hippocampus-dir))
      (cons 'ok (list :entries nil :count 0))
    (let* ((files (directory-files dl-satan-hippocampus-dir t "\\.org\\'"))
           (entries (mapcar #'dl-satan-tools-hippocampus--entry files))
           (sorted (sort entries
                         (lambda (a b)
...
(defun dl-satan-tools-hippocampus--safe-path-p (filename)
...
(defun dl-satan-tool/hippocampus-read (args _ctx)
  ...
  (let ((filename (plist-get args :filename)))
    (cond
     ...
     (t
      (let ((path (expand-file-name filename dl-satan-hippocampus-dir)))
...
(defun dl-satan-tools-hippocampus--replace-body (path new-body)
...
(defvar dl-satan-tools-hippocampus--rg-program "rg"
...
(defun dl-satan-tool/hippocampus-grep (args ctx)
  ...
  (let ((query (plist-get args :query)))
    (cond
     ...
     ((not (file-directory-p dl-satan-hippocampus-dir))
     ...
     (t
      (let* ((stdout-buf (generate-new-buffer " *satan-hippo-rg*"))
             ...
             (argv (list "--no-heading" "--line-number"
                         ...
                         "--max-columns" "200"
                         "--ignore-case"
                         "--" query dl-satan-hippocampus-dir))
             (exit (apply #'call-process
                          dl-satan-tools-hippocampus--rg-program nil
        ...
        (if (and (not (= exit 0)) (not (= exit 1)))
          ...
          (let* ((lines (cl-remove-if #'string-empty-p
                 ...
                 (capped (if (> (length lines)
                                dl-satan-tools-hippocampus--grep-max)
                             (cl-subseq lines 0
                                        dl-satan-tools-hippocampus--grep-max)
                           lines))
                 (matches
...
(defun dl-satan-tools-hippocampus--update-title-header (path new-title)
...
(defun dl-satan-tool/hippocampus-rename (args ctx)
  ...
  (let ((filename (plist-get args :filename))
    ...
    (cond
     ...
     (t
      (let ((old-path (expand-file-name filename dl-satan-hippocampus-dir)))
        (if (not (file-exists-p old-path))
            (cons 'error (format "not found: %s" filename))
          (let* ((new-slug (dl-satan-tools-hippocampus--slugify new-title))
                 (new-filename
                 ...
                 (new-path (expand-file-name new-filename
                                             dl-satan-hippocampus-dir)))
            (dl-satan-tools-hippocampus--update-title-header
             old-path new-title)
...
(dl-satan-tool-register
 (list :name "hippocampus_grep"
       ...
       :handler 'dl-satan-tool/hippocampus-grep))
