;;; dl-satan-pattern.el --- pattern definitions + rebuild projection -*- lexical-binding: t; -*-
...
;;
;; Public surface:
;;   (dl-satan-pattern-sync &optional PATTERNS-FILE DB)
...
;;
;;   (dl-satan-pattern-rebuild &optional DB)
...
;;
;;   (dl-satan-pattern-stats &optional DB)
;;     Query satan_pattern_stats view → list of plists.
;;
;;   (dl-satan-pattern-list &optional DB)
;;     Query satan_patterns → list of plists.
;;
;;   (dl-satan-pattern-scars &optional PATTERN-ID DB)
...
(defcustom dl-satan-pattern-file
...
(defun dl-satan-pattern--parse-handle (handle)
  ...
  (let ((pos (string-match ":" handle)))
    ...
    (let ((ns (substring handle 0 pos))
      ...
      (unless (dl-satan-memory-grammar-namespace-world (intern ns))
        (user-error "pattern handle %S: unknown namespace %S" handle ns))
      (unless (dl-satan-memory-grammar-valid-value-p (intern ns) val)
        (user-error "pattern handle %S: invalid value %S for namespace %S"
                    handle val ns))
      (cons ns val))))

(defun dl-satan-pattern--validate-handles (handles pattern-id)
  ...
  (let ((hs (if (vectorp handles) (append handles nil) handles)))
    (unless (and (listp hs) (cl-every #'stringp hs))
      (user-error "pattern %S: :cue_handles must be list of strings, got %S"
                  pattern-id handles))
    (dolist (h hs)
      (dl-satan-pattern--parse-handle h)))
  t)

;; ── SQL helpers ─────────────────────────────────────────────────────────────

(defun dl-satan-pattern--quote-text (s)
  "Return the SQL literal for S; nil → NULL."
  (cond
   ((null s) "NULL")
   ((stringp s)
    (concat "'" (replace-regexp-in-string "'" "''" s) "'"))
   (t (error "dl-satan-pattern--quote-text: not stringy: %S" s))))

(defun dl-satan-pattern--quote-jsonb (obj)
  "Serialize OBJ as JSON, wrapping as `'…'::jsonb'.
Converts elisp lists to vectors via `dl-satan-jsonl-prepare' so they
serialize as JSON arrays, matching the pattern used by the intervention
layer."
  (let* ((prepared (dl-satan-jsonl-prepare (if obj obj (vector))))
         (coded (json-serialize prepared
                                :null-object :null
                                :false-object :false)))
    (concat (dl-satan-pattern--quote-text coded) "::jsonb")))

(defun dl-satan-pattern--exec-sql (db sql &optional extra-flags)
  "Run SQL through psql --single-transaction.  Signals on failure."
  (let ((flags (append (or extra-flags '())
                       (list "--single-transaction" "-f" "-"))))
    (pcase (dl-satan-db-psql db dl-satan-memory-migrate-host
                             dl-satan-memory-migrate-psql-program
                             flags sql)
      (`(ok . ,_) nil)
      (`(error . ,msg)
       (user-error "dl-satan-pattern SQL: %s" msg)))))
...
(defun dl-satan-pattern--validate-definition (entry)
...
(defun dl-satan-pattern--upsert-sql (entry updated-at)
  ...
  (concat
   ...
   (mapconcat #'identity
              (list (dl-satan-pattern--quote-text (plist-get entry :id))
                    (dl-satan-pattern--quote-text (plist-get entry :label))
                    (dl-satan-pattern--quote-jsonb (plist-get entry :cue_handles))
                    (dl-satan-pattern--quote-text (plist-get entry :default_intervention))
                    (dl-satan-pattern--quote-text (plist-get entry :intrusion_ceiling))
                    (number-to-string (or (plist-get entry :priority) 0))
                    (if (plist-get entry :enabled) "true" "false")
                    (dl-satan-pattern--quote-text (plist-get entry :notes))
                    (concat (dl-satan-pattern--quote-text updated-at)
                            "::timestamptz"))
              ", ")
  ...
  (let* ((db (or db dl-satan-memory-migrate-database))
    ...
    (let ((upserted 0))
      (dolist (entry definitions)
        (let ((id (dl-satan-pattern--validate-definition entry)))
          (puthash id t ids-in-file)
          (dl-satan-pattern--exec-sql
           db (dl-satan-pattern--upsert-sql entry now))
          (cl-incf upserted)))
      ;; Soft-retire patterns not in file
      (let ((retired 0))
        (cl-loop for p in (dl-satan-pattern-list db)
                 do (unless (gethash (plist-get p :id) ids-in-file)
                      (dl-satan-pattern--exec-sql
                       db (concat
                           "UPDATE satan_patterns SET enabled = false, "
                           "updated_at = "
                           (dl-satan-pattern--quote-text now)
                           "::timestamptz WHERE id = "
                           (dl-satan-pattern--quote-text
                            (plist-get p :id))))
                      (cl-incf retired)))
...
(defconst dl-satan-pattern--rebuild-lock-key 900876543
  "Advisory lock key for pattern-outcome rebuild (arbitrary bigint).")

(defun dl-satan-pattern-rebuild (&optional db)
  ...
  (let* ((db (or db dl-satan-memory-migrate-database))
         (script
          (concat
           ...
           (format "SELECT pg_advisory_xact_lock(%d);\n"
                   dl-satan-pattern--rebuild-lock-key)
    ...
    (let* ((count-sql "SELECT COUNT(*)::text FROM satan_pattern_outcomes")
      ...
      (pcase result
        ...
        (`(error . ,msg)
         (user-error "dl-satan-pattern-rebuild count: %s" msg))))))

;; ── read accessors ──────────────────────────────────────────────────────────

(defun dl-satan-pattern--query-with-cols (db cols sql)
      ...
      (`(ok . ,out)
       (cl-loop for line in (split-string (string-trim out) "\n" t)
                ...
                collect (cl-mapcan
                         ...
                         keys cells)))
      (`(error . ,msg)
       (user-error "dl-satan-pattern query: %s" msg)))))

(defun dl-satan-pattern-stats (&optional db)
  ...
  (let ((db (or db dl-satan-memory-migrate-database))
    ...
    (dl-satan-pattern--query-with-cols
     db cols
     "SELECT * FROM satan_pattern_stats ORDER BY pattern_id")))

(defun dl-satan-pattern-list (&optional db)
...
(defun dl-satan-pattern-scars (&optional pattern-id db)
     ...
     (concat
      ...
      (when pattern-id
        (concat " AND pattern_id = "
                (dl-satan-pattern--quote-text pattern-id)))
      " ORDER BY ts DESC"))))

(provide 'dl-satan-pattern)
;;; dl-satan-pattern.el ends here
